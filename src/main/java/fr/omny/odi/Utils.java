...
public class Utils {
  ...
  private static Set<OnConstructorCallListener> constructorCallListeners =
      new HashSet<>();
   ...
   */
  public static void
  registerCallConstructor(OnConstructorCallListener listener) {
  ...
  public static <T> T callConstructor(Constructor<?> constructor,
    ...
    for (int i = 0; i < finalParameters.length; i++) {
      ...
      if (targetParam.isAnnotationPresent(Autowired.class)) {
        ...
        var autowireObj =
            Injector.getService(targetParam.getType(), autowiredData.value());
  ...
  public static List<Class<?>> getClasses(List<String> classes,
    ...
    Injector.getLogger().ifPresent(
        log
        -> log.info("Analyzed " + classes.size() + " class files in " +
    ...
    for (Field field : klass.getDeclaredFields()) {
      if (field.isAnnotationPresent(Autowired.class)) {
        ...
        if (field.getType() == Optional.class) {
          ...
          serviceInstance =
              Injector.getService(serviceType, autowiredData.value());
          // serviceInstance
          field.set(instance, Optional.ofNullable(serviceInstance));
          fieldType = serviceType;
        } else {
          serviceInstance =
              Injector.getService(field.getType(), autowiredData.value());
