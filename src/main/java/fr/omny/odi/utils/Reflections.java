...
public final class Reflections {
	...
	/**
	 * Read bytecode class declaration without loading dependencies, without loading
	 * code and all
	 * 
	 * @param path
	...
	public static PreClass readPreClass(String path) {
		...
		try (var inputStream = classLoader.getResourceAsStream(path)) {
			...
			// Reading information about the class (Methods, Annotations, Super class
			// extends, etc...)
			...
			return new PreClass(packageName, path.replace('/', '.'), harvester);
		} catch (Exception e) {
			e.printStackTrace();
